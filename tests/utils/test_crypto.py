...
def test_create_key_pair():
    secp256k1_private_key, secp256k1_public_key = crypto.create_key_pair(
        private_key=None, compressed=False
    )
    coincurve_private_key, coincurve_public_key = crypto2.create_key_pair(
...
def test_sign_recoverable():
    ...
    secp256k1_recoverable_sign: bytes = crypto.sign_recoverable(
        message, secp256k1_private_key
    )
    coincurve_recoverable_sign: bytes = crypto2.sign_recoverable(
        message, secp256k1_private_key
    )
